...
interface AICopilotProps {
  ...
  onExportCsvRequest?: (dataType: string) => void;
}

export default function AICopilot({ dashboardContext, onExportCsvRequest }: AICopilotProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<{role: 'user' | 'ai', content: string}[]>([
    { role: 'ai', content: 'Halo! Saya AI Data Analyst Anda. Ada yang bisa saya bantu terkait laporan keuangan atau data pengunjung hari ini?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.reply) {
        ...
        // Jika backend mengirim CSV data, langsung download
        if (data.csvData) {
          const blob = new Blob([data.csvData], { type: 'text/csv;charset=utf-8;' });
          const url = URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.download = `Laporan_Aviary_${new Date().toLocaleDateString('id-ID').replace(/\//g, '-')}.csv`;
          link.click();
        ...
        setMessages(prev => [...prev, { role: 'ai', content: "Terjadi kesalahan saat memproses data." }]);
      }
    } catch (err) {
      console.error("Copilot Error:", err);
      setMessages(prev => [...prev, { role: 'ai', content: "Koneksi ke AI terputus. Pastikan API Key valid dan koneksi internet stabil." }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: '0.75rem', opacity: 0.8 }}>AI Data Analyst • Aviary Park</p>
            </div>
          </div>
